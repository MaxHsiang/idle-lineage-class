...
assert.ok(gameHtml.includes('js/genesis/genesis-loader.js?v=1.6.22-ignore-defense-unlimited-panacea'), 'static genesis loader bootstrap missing');
...
assert.strictEqual(Object.keys(Genesis.items.definitions).length, 38);
assert.strictEqual(Genesis.items.loadout.length, 38);
...
assert.strictEqual(Object.values(player.eq).filter(Boolean).length, 38);
assert.ok(player.skills.includes('sk_test'));
assert.strictEqual(player.genesisRuntimeVersion, Genesis.config.version);

console.log('Genesis smoke test passed: 38 definitions, two full equipment pages, skills, affixes.');
