...
function _playerMorphApply() {   // 8fps ticker 驅動（🗡️ v3.0.67 形態＝變身優先→職業動態·key 含武器變體→換武器自動重建）
    ...
    // 🚶 v3.7.64 主角走位（取代 v3.6.89 玩家固定站位·僅主玩家；傭兵仍固定站位）：_playerMoveStep 每輪步進·CSS 補間平滑（同寵物 .14s linear）
    {
        ...
        if (inTown) {
            ...
            _pmState.el.style.bottom = '12%';
            _pmState.el.style.zIndex = '520';
    ...
    if (!inTown && !_pmState.moving && (CLASS_ANIM_8DIR.has(player.avatar) || MORPH_ANIM_3DIR.has(_playerMorphName() || ''))) _classFacing8(player, _pmState.el);
    ...
    if (act === null) {
        let mv = !inTown && _pmState.moving && a.walk && a.walk.length;
        let base = mv ? a.walk : a.idle;
        if (base) { act = mv ? 'walk' : 'idle'; f = inTown ? 0 : Math.floor(Date.now() / (1000 / MOB_ANIM_FPS)) % base.length; _useW = false; }
    }
    if (act === null) return;
