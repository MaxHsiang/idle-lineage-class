...
function recomputeStats() {
    ...
    if (p.cls === 'omni' && typeof DB !== 'undefined' && DB.skills) {
        let omniSkills = Object.keys(DB.skills).filter(id => id !== 'sk_fireball_burst' && !DB.skills[id].procOnly);
        player.skills = Array.from(new Set((player.skills || []).concat(omniSkills)));
        player.grantedSkills = omniSkills.slice();
    }
    ...
    if (p.cls === 'omni') {
        d.mr = 30;
        d.ac -= Math.floor(p.lv / 5);
        d.mr += Math.floor(p.lv / 2);
        d.er += Math.floor(p.lv / 4);
        d.meleeDmg += Math.floor(p.lv / 3);
        d.rangedDmg += Math.floor(p.lv / 3);
        d.extraDmg += Math.floor(p.lv / 3);
        d.extraHit += Math.floor(p.lv / 3);
    ...
    let hpBase   = p.cls === 'omni' ? 18 : (p.cls === 'knight' ? 16 : (p.cls === 'elf' ? 15 : (p.cls === 'dark' ? 12 : (p.cls === 'illusion' ? 14 : (p.cls === 'dragon' ? 16 : (p.cls === 'warrior' ? 16 : (p.cls === 'royal' ? 14 : 12)))))));
    let hpClsInc = p.cls === 'omni' ? 10 : (p.cls === 'knight' ? 8.5 : (p.cls === 'elf' ? 7.3 : (p.cls === 'dark' ? 10.5 : (p.cls === 'illusion' ? 7.5 : (p.cls === 'dragon' ? 4.5 : (p.cls === 'warrior' ? 9 : (p.cls === 'royal' ? 10 : 4.3)))))));
    let conInc   = getConGrowth(d.con, p.cls);
    p.mhp = hpBase + (p.lv - 1) * (hpClsInc + conInc);   // 小數隱藏不捨去（顯示時取floor）

    let mpBase   = p.cls === 'omni' ? 10 : (p.cls === 'knight' ? 1 : (p.cls === 'elf' ? 5 : (p.cls === 'dark' ? 3 : (p.cls === 'illusion' ? 5 : (p.cls === 'dragon' ? 2 : (p.cls === 'warrior' ? 1 : (p.cls === 'royal' ? 2 : 6)))))));
    let mpClsInc = p.cls === 'omni' ? 6 : (p.cls === 'knight' ? 1 : (p.cls === 'elf' ? 2.83 : (p.cls === 'dark' ? 3 : (p.cls === 'illusion' ? 3.2 : (p.cls === 'dragon' ? 0.7 : (p.cls === 'warrior' ? 0.5 : (p.cls === 'royal' ? 1.5 : 4.5)))))));
...
d.mr += (baseMr + bonusMr);
    ...
    // 席琳＝所有套裝合一：席琳件數同時視為十二個原套裝各自的件數。
    let _shN = (g) => Math.max((_shSets[g] || 0), (_shSets['席琳'] || 0));
    ...
    { let _clvW = p.eq.wpn ? DB.items[p.eq.wpn.id] : null; let _clvOn = !p.classicMode && ((p.statuses && p.statuses.cleave > 0) || (entityHasMastery(p, 'k_cleave') && _clvW && _clvW.eff === 'cleave')); if(_clvOn) spdMult *= (entityHasMastery(p, 'k_cleave') ? 0.50 : 0.80); }
    { let _swMelee = p.eq.wpn ? DB.items[p.eq.wpn.id] : null; if(entityHasMastery(p, 'e_sword') && _swMelee && !_swMelee.w2h && !_swMelee.isBow && !_swMelee.ranged) spdMult *= (1/1.5); }
    { let _aw = p.eq.wpn ? getWeaponTags(p.eq.wpn.id) : []; let _ow = p.eq.offwpn ? getWeaponTags(p.eq.offwpn.id) : []; if(entityHasMastery(p, 'k_giantaxe') && (_aw.includes('雙手鈍器') || _ow.includes('雙手鈍器'))) spdMult *= (1/1.3); else if(entityHasMastery(p, 'k_dualaxe') && _aw.includes('單手鈍器') && p.eq.offwpn && _ow.includes('單手鈍器')) spdMult *= (1/1.3); }
    { let _rw = p.eq.wpn ? getWeaponTags(p.eq.wpn.id) : []; if(entityHasMastery(p, 'k_royal_sword') && (_rw.includes('單手劍') || _rw.includes('雙手劍'))) spdMult *= (1/1.5); }
    { let _iw = p.eq.wpn ? DB.items[p.eq.wpn.id] : null; if((p.cls === 'illusion' || p.cls === 'omni') && _iw && !_iw.isBow && ((entityHasMastery(p, 'i_qigu') && _iw.qigu) || (entityHasMastery(p, 'i_magicsword') && !_iw.qigu && !isWandWeapon(_iw)))) spdMult *= (1/1.3); }
    ...
    if(p.buffs.sk_elf_singleres > 0 && p.elfEle) {
        if(p.elfEle === 'fire'  || p.elfEle === 'all') d.resFire  += 50;
        if(p.elfEle === 'water' || p.elfEle === 'all') d.resWater += 50;
        if(p.elfEle === 'earth' || p.elfEle === 'all') d.resEarth += 50;
        if(p.elfEle === 'wind'  || p.elfEle === 'all') d.resWind  += 50;
    }
...
const POLY_TIERS = [
    { min:0, max:9, color:"text-white", forms:[
        { n:"終極死亡騎士", lv:1, omniOnly:true, apm:{ '單手劍':300,'單手鈍器':300,'雙手鈍器':300,'弓':300,'十字弓':300,'單手矛':300,'雙手矛':300,'魔杖':300,'匕首':300,'雙手劍':300,'雙刀':300,'鋼爪':300,'奇古獸':300,'鎖鏈劍':300,'雙斧':300 }, wlk:4, cast:1, stun:0, ed:25, eh:25, mgd:25, sp:25 },
...
function getPolyState(isDKSet=false, isKurtSet=false) {
    ...
    let forms = tier.forms.filter(f => !f.omniOnly || (player && player.cls === 'omni'));
...
function openPolySelect(uid) {
    ...
    let avail = [];
    for (const t of POLY_TIERS) for (const f of t.forms) if (player.lv >= f.lv && (!f.omniOnly || player.cls === 'omni')) avail.push({ f, color: f.omniOnly ? 'text-fuchsia-300' : t.color });
    avail.sort((a, b) => (b.f.lv - a.f.lv) || (a.f.atk - b.f.atk));
    listEl.innerHTML =
        '<button class="btn text-left !py-2 !px-3 border-red-500/70" onclick="cancelCurrentPoly()">' +
            '<span class="text-red-300 font-bold">取消目前變身</span>' +
            '<span class="text-slate-400 text-xs block mt-0.5">恢復角色原本外觀，不消耗變形卷軸。</span>' +
        '</button>' +
        avail.map(({ f, color }) =>
...
function cancelCurrentPoly() {
    if (!player) { closePolyModal(); return; }
    player.poly = null;
    if (player.buffs) {
        player.buffs.poly = 0;
        if (typeof OMNI_AWAKEN_SKILL_ID !== 'undefined') player.buffs[OMNI_AWAKEN_SKILL_ID] = 0;
    }
    if (typeof _playerMorphRemove === 'function') { try { _playerMorphRemove(); } catch (e) {} }
    calcStats(); closePolyModal(); updateUI();
    if (typeof saveGame === 'function') saveGame();
    logSys('<span class="text-slate-300">已取消變身，恢復角色原本外觀。</span>');
}
